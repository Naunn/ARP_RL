...
def create_dist_dict(cities: list[str] = []):
    ...
    # 1. Pre-fetch coordinates to avoid redundant API calls
    coords_cache = {}
    for city in cities:
        location = geolocator.geocode(city)
        if location:
            coords_cache[city] = (location.latitude, location.longitude)
        else:
            print(f"Warning: Could not find coordinates for {city}")

    dist_dict = {}

    # 2. Calculate distances for all unique pairs (A to B)
    ...
    # 3. Add zero-distance for identity pairs (A to A)
    for city in cities:
...
cities = ["lodz", "torun", "milan", "paris", "london"]
create_dist_dict(cities)
